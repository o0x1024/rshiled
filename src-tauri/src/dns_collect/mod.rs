...
pub mod datasets;
use certificates::certspotter::get_certspotter_subdomains;
// use tokio;
...
pub async fn dns_collection_by_api(domain: &str) -> Result<Vec<String>, String> {
    // let ip138s = datasets::ip138::get_ip138_subdomains(base_url);
    let mut domains = Vec::new();
    match get_dns_collection_func() {
        Ok(func_map) => {
            for (key, mfunc) in func_map {
...
mod tests {
    ...
    async fn test_dns_collection_by_api() {
        ...
        dns_collection_by_api(base_url).await;
    }
}
