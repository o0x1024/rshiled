				...
				<div class="menu-wrapper">
					<a-menu :style="{ height: 'calc(100% - 0px)' }"  @collapse="setCollapsed" @menu-item-click="onMenuClick" mode="pop"
						...
						<a-menu-item key="1">
							...
							{{ $t('repeater.repeater_main') }}
						</a-menu-item>
						<a-menu-item key="2">
							<template #icon><icon-book /></template>
							{{ $t('repeater.repeater_history') }}
						</a-menu-item>
					</a-menu>
				</div>
			</a-layout-sider>
...
const cachedViews = computed(() => {
  ...
  console.log('Repeater cached views:', cacheList);
  return cacheList;
});
...
watch(() => route.name, () => {
	...
	// 保存当前子菜单路由名称（如果不是根路由）
	if (route.name && route.name.toString() !== 'repeater') {
...
onActivated(() => {
	...
	if (route.name === 'repeater' && lastVisitedRoute.value) {
		router.push({ name: lastVisitedRoute.value });
	} else if (route.name === 'repeater' && !lastVisitedRoute.value) {
		// 如果没有记忆的路由，则使用默认值
		router.push({ name: "repeater-main" });
	}
});
...
const onMenuClick = (key: string) => {
	switch (key) {
		case '1':
			...
			break
		case '2':
			router.push({ name: "repeater-history" });
			break
	}
}
